...
def mobile_bottom_navigation(pages):
    """
    Cria navegação inferior para mobile
    
    Args:
        pages: Lista de dicionários com 'name', 'icon', 'url'
    """
    nav_html = """
    ...
    for page in pages:
        active_class = "active" if page.get('active', False) else ""
        nav_html += f"""
        <a href="{page['url']}" class="nav-item {active_class}">
